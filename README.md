...
- [Scalability and System Design](#scalability-and-system-design)
...
In older operating systems (in newer paging is used, scroll down): the hardware provides the **base and bounds** 
registers; each CPU thus has an additional pair of registers, part of the 
...
to use a queue with waiting thread IDs. A guard is used as a spin lock, then the lock is acquired if it's available,
